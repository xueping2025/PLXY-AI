...
def detect_vessels(image_folder, weight_file, output_folder, progress_callback=None):
    ...
    excel_output = os.path.join(output_folder, 'vessel10X_detection_results.xlsx')
    ...
    for idx, img_file in enumerate(image_files, start=1):
        ...
        detections = sorted(detections, key=lambda x: (x[1] + x[3]) / 2)  # Sort by the center y (vertical)
        ...
        # Prepare a list for each image to store vessel data
        image_vessel_data = []
        ...
        for i, det in enumerate(detections):
            ...
            # Draw rectangle for each detected vessel
            ...
            # Store vessel number, real width, real height, diameter, and area in the excel data
            image_vessel_data.append([img_file, i + 1, width_real, height_real, diameter, area])

        # Display the vessel count in the top-left corner
        cv2.putText(image, f'Count: {vessel_count}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        ...
        excel_data.append([img_file, 'Total', None, None, None, vessel_count])
        # Add detailed vessel data
        excel_data.extend(image_vessel_data)
    ...
    df = pd.DataFrame(excel_data, columns=['Image Name', 'Vessel Number', 'Width (microns)', 'Height (microns)', 'Diameter (microns)', 'Area (square microns)'])
