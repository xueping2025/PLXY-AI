...
def detect_ray_cross(image_folder, weight_file, output_folder, progress_callback=None):
    ...
    excel_output = os.path.join(output_folder, 'ray_cross40X_detection_results.xlsx')
    ...
    for idx, img_file in enumerate(image_files, start=1):
        ...
        for i, det in enumerate(detections):
            ...
            area = (width * height) * (pixel_to_micron ** 2)  # µm^2
            diameter = max(width, height) * pixel_to_micron  # µm
            ...
            # Clamp horizontal位置到 [x_min, x_max - tw]
            org_x = max(x_min, min(org_x, x_max - tw))

            # 垂直约束：top >= y_min => org_y >= y_min + th
            # bottom <= y_max => org_y - baseline? OpenCV putText y is baseline, so ensure baseline <= y_max
            min_org_y = y_min + th
            max_org_y = y_max - baseline
            if min_org_y > max_org_y:
                # 小框退化处理：把 org_y 限在 [y_min + th, y_max]
                org_y = max(y_min + th, min(org_y, y_max))
            else:
                org_y = max(min_org_y, min(org_y, max_org_y))

            # Put the text (red) with fixed thickness
            cv2.putText(image, label, (org_x, org_y), font, font_scale, (0, 0, 255), fixed_thickness, cv2.LINE_AA)

        # Calculate averages
        if ray_cross_count > 0:
            avg_width = total_width / ray_cross_count
            avg_height = total_height / ray_cross_count
            avg_diameter = total_diameter / ray_cross_count
            avg_area = total_area / ray_cross_count
        else:
            avg_width = avg_height = avg_diameter = avg_area = 0.0

        # Append the image-level summary to excel_data
        excel_data.append([img_file, ray_cross_count, avg_width, avg_height, avg_diameter, avg_area])

        # Display the ray-cross count in the top-left corner
        cv2.putText(image, f'Count: {ray_cross_count}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)
    ...
    df = pd.DataFrame(excel_data, columns=[
        'Image Name', 'Ray-Cross Count', 'Average Width (microns)', 'Average Height (microns)',
